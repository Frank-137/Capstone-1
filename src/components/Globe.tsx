...
const Globe: React.FC<GlobeProps> = ({
  ...
  useEffect(() => {
    ...
    if (!geoJsonCache.current) {
      ...
      if (sceneRef.current) {
        drawContinentBorders(geoJsonCache.current);
      }
    }
    ...
    // Add raycaster for interactivity (แบบโค้ด 2)
    const raycaster = new THREE.Raycaster();
    const mouse = new THREE.Vector2();

    const onClick = (event: MouseEvent) => {
      if (!renderer || !cameraRef.current || !pinGroupRef.current) return;
      const bounds = renderer.domElement.getBoundingClientRect();
      mouse.x = ((event.clientX - bounds.left) / bounds.width) * 2 - 1;
      mouse.y = -((event.clientY - bounds.top) / bounds.height) * 2 + 1;
      raycaster.setFromCamera(mouse, cameraRef.current);
      const intersects = raycaster.intersectObjects(pinGroupRef.current.children);
